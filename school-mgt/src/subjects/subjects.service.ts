...
export class SubjectsService {
  ...
  async create(createSubjectDto: CreateSubjectDto) {
    ...
    try {
      const subject = await this.dataSource.transaction(async (manager) => {
  ...
  async findAll(query: QuerySubjectDto) {
    ...
    if (query.isCore !== undefined) {
      queryBuilder.andWhere('subject.isCore = :isCore', {
    ...
    queryBuilder.orderBy('subject.createdAt', 'DESC').skip(skip).take(limit);
  ...
  async update(id: number, dto: UpdateSubjectDto) {
    ...
    Object.assign(subject, {
      ...dto,
      code: dto.code ? dto.code.toUpperCase() : subject.code,
    });
